...
`git clone https://github.com/ByteLegendQuest/java-write-javadoc`

A project directory named `java-write-javadoc` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-write-javadoc` directory which is created just now.
